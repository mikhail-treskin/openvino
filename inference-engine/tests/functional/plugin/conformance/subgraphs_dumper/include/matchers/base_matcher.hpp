...
public:
    using clone_fn = std::function<const std::shared_ptr<ngraph::Node>(const std::shared_ptr<ngraph::Node> &, OPMetaInfo &)>;
    ...
    std::map<ngraph::NodeTypeInfo, clone_fn> cloners_map = {};
